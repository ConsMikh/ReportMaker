...
class LinksPartMaker(PartMaker):
    ...
    def process(self):
        self.log.info(
            f"Начало формирования части со связями")
        ...
        self.log.info(
            f"Часть со связями сформирована")
    ...
    def make_str_links(self):
        link_str = []
        link_str.append(
            f"Даты, в которые были записи о помидорках\n")
...
class PeriodLinksPartMaker(LinksPartMaker):
    ...
    def set_file_lists(self):
        self.report['links']['list_create'] = self.set_list_file(
            os.path.getctime)
        self.report['links']['list_change'] = self.set_list_file(
            os.path.getmtime)

    def set_list_file(self, func):

        kbase_files = [f for f in os.listdir(self.kbase_path) if os.path.isfile(
            os.path.join(self.kbase_path, f))]
        kbase_files_list = []
        epoch = datetime.datetime.utcfromtimestamp(0)

        start_date = datetime.datetime.fromisoformat(
            self.report['period']['start_date'])
        start_sec = (start_date - epoch).total_seconds()
        end_date = datetime.datetime.fromisoformat(
            self.report['period']['end_date'])
